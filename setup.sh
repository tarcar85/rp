...
for x in api ;
 do
  ...
  ssh $x ' sudo yum install git -y && git clone https://github.com/secobau/nodejs && cd nodejs/rest-api ' ;
  ssh $x ' rm package.json && npm init --yes && npm install express && sed -i /app.listen/d index.js && echo app.listen(8080) 1>>index.js ' ;
 done ;
